...
export default async function ProjectPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <article className={`projectPage projectPage--${family}`}>
    ...
    <div className="projectBody"><aside className="stickyFacts"><dl><div><dt>Планировка</dt><dd>{layoutNames[project.layout]}</dd></div><div><dt>Помещение</dt><dd>{project.room}</dd></div><div><dt>Основные размеры</dt><dd>{project.dimensions}</dd></div><div><dt>Фасад</dt><dd>{project.facade}</dd></div><div><dt>Столешница</dt><dd>{project.top}</dd></div></dl><Link className="button" href={`/calculator?layout=${project.layout}&facade=${config.facade}&top=${config.top}&a=3000`}>Взять за основу</Link></aside>
      <div className="projectStory"><section><p className="eyebrow">Задача</p><h2>{project.task}</h2><p>{project.description}</p></section><div className="storyGallery">{project.gallery.slice(1).map(image=><figure key={image.src}><div><Image src={image.src} alt={image.alt} fill sizes="(max-width: 700px) 100vw, 50vw"/></div><figcaption>{image.caption}</figcaption></figure>)}</div><p className="galleryNote">Материальная подборка. Кадры показывают сочетание фасада, столешницы и света, а не фотографии одного реализованного объекта.</p><section><h2>Хранение и детали</h2><p>{project.storage}</p><ul>{project.features.map(feature=><li key={feature}>{feature}</li>)}</ul></section><section><h2>Условная 2D-схема</h2><KitchenDiagram config={config}/></section></div></div>
