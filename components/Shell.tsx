...
export function Header(){
  ...
  return <header className="header"><div className="brand"><Link className="logo" href="/">СЛОЙ <b>52</b></Link><a className="profileLink" href="https://vk.ru/george_5" target="_blank" rel="noreferrer">VK · George</a></div><button ref={menuButton} className="menuBtn" aria-expanded={open} aria-controls="nav" onClick={()=>setOpen(value=>!value)}>{open?'Закрыть':'Меню'}</button><nav ref={navigation} id="nav" className={open?'nav open':'nav'} aria-label="Основная навигация">{links.map(([href,label])=><Link key={href} href={href} aria-current={active(href)?'page':undefined} onClick={close}>{label}</Link>)}<Link className="button small" href="/contacts" aria-current={pathname==='/contacts'?'page':undefined} onClick={close}>Обсудить проект</Link></nav></header>;
}
