...
export function RoomRail(){
  ...
  return <section className="roomSection section" aria-labelledby="rooms-title">
    <div className="roomSectionHead"><div><p className="eyebrow">Навигация по дому</p><h2 id="rooms-title">Мебель по комнатам</h2></div><p>Не большой каталог, а шесть точек входа: от кухни и гардеробной до рабочей ниши.</p><div className="railArrows"><button type="button" onClick={()=>move(-1)} aria-label="Показать предыдущие комнаты">←</button><button type="button" onClick={()=>move(1)} aria-label="Показать следующие комнаты">→</button></div></div>
    <div className="roomRail" ref={rail}>{rooms.map((room,index)=><Link className={`roomCard roomCard--${index%3}`} href={room.href} key={room.name}><div className="roomCardImage"><Image src={room.image} alt={room.alt} fill sizes="(max-width: 700px) 78vw, 34vw"/></div><span className="roomIndex">{String(index+1).padStart(2,'0')}</span><h3>{room.name}</h3><p>{room.note}</p><b aria-hidden="true">↗</b></Link>)}</div>
  </section>;
}
