...
export function MaterialScene({config}:{config:KitchenConfig}){return <div className="materialScene" role="region" aria-label="Крупный preview выбранного сочетания"><KitchenFront config={config} label="Кухонная сцена выбранных материалов"/><div className="materialSamples" aria-label="Образцы выбранных материалов"><div><span className={`detailFacade ${config.facade}`}/><b>Фасад</b></div><div><span className={`detailTop ${config.top}`}/><b>Столешница</b></div><div><HandlePreview type={config.handle}/><b>Ручка</b></div></div></div>}
